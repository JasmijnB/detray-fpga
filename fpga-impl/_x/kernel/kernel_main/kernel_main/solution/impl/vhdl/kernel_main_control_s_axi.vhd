...
entity kernel_main_control_s_axi is
generic (
...
port (
    ...
    b_ptr                 :out  STD_LOGIC_VECTOR(63 downto 0);
...
end entity kernel_main_control_s_axi;
...
-- 0x2c : reserved
-- 0x30 : Data signal of b_ptr
--        bit 31~0 - b_ptr[31:0] (Read/Write)
-- 0x34 : Data signal of b_ptr
--        bit 31~0 - b_ptr[63:32] (Read/Write)
-- 0x38 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of kernel_main_control_s_axi is
    ...
    constant ADDR_B_PTR_DATA_0 : INTEGER := 16#30#;
    constant ADDR_B_PTR_DATA_1 : INTEGER := 16#34#;
    constant ADDR_B_PTR_CTRL   : INTEGER := 16#38#;
    ...
    signal int_b_ptr           : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_B_PTR_DATA_0 =>
                        rdata_data <= RESIZE(int_b_ptr(31 downto 0), 32);
                    when ADDR_B_PTR_DATA_1 =>
                        rdata_data <= RESIZE(int_b_ptr(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    b_ptr                <= STD_LOGIC_VECTOR(int_b_ptr);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_b_ptr(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_B_PTR_DATA_0) then
                    int_b_ptr(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_b_ptr(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_b_ptr(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_B_PTR_DATA_1) then
                    int_b_ptr(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_b_ptr(63 downto 32));
                end if;
            end if;
        end if;
    end process;
