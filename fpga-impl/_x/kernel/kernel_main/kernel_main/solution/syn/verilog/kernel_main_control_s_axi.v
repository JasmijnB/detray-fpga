...
// ==============================================================
`timescale 1ns/1ps
module kernel_main_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_B_PTR_DATA_0 = 6'h30,
    ADDR_B_PTR_DATA_1 = 6'h34,
    ADDR_B_PTR_CTRL   = 6'h38,
    WRIDLE            = 2'd0,
    WRDATA            = 2'd1,
    WRRESP            = 2'd2,
    WRRESET           = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_B_PTR_DATA_0: begin
                    rdata <= int_b_ptr[31:0];
                end
                ADDR_B_PTR_DATA_1: begin
                    rdata <= int_b_ptr[63:32];
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_B_PTR_DATA_0)
            int_b_ptr[31:0] <= (WDATA[31:0] & wmask) | (int_b_ptr[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_B_PTR_DATA_1)
            int_b_ptr[63:32] <= (WDATA[31:0] & wmask) | (int_b_ptr[63:32] & ~wmask);
    end
end
